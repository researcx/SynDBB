...
$("#pwform").submit(function(event) {
  var crypt = new Crypt();
  var plain_pw1 = $("#oldPassword").val();
  var plain_pw2 = $("#inputPassword").val();
  var confirm_pw2 = $("#confirmPassword").val();
  var oldpassword = CryptoJS.SHA256(plain_pw1);
  ...
  $("#oldpword").val(plain_pw1);
  $("#newpword").val(plain_pw2);
  $("#newpword_hash").val(newpassword);

  if(plain_pw2 == confirm_pw2) {

  $.ajax({
         type: "POST",
         url: "/functions/change_password",
         ...
         success: function(data)
         {
           if(data == "Password change successful."){
             var dialog = bootbox.dialog({
                 ...
                 message: '<i class="fa fa-spin fa-cog fa-fw" style="margin-right: 5px;"></i> Password change successful. Redirecting to the main page... ',
                 closeButton: false
             });
